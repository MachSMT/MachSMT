...
OLD_DB_INDEXER = True
def reverter(bench):
    return bench.replace('benchmarks/smt-lib/incremental/', 'benchmarks/').replace('benchmarks/smt-lib/non-incremental/','benchmarks/')
...
class MachSMT:
                ...
                for logic in self.smtcomp_data[year][track]:
                    ...
                    for solver in self.smtcomp_data[year][track][logic]:
                        ...
                        if len(comp_benchmarks) - len(solver_benchmarks) != 0:
                            warning("Solver [" + solver + '] missing: ' + str(len(comp_benchmarks) - len(solver_benchmarks)) + "/" + str(len(comp_benchmarks)) + " benchmarks.",logic,track,year)
                    if len(comp_benchmarks) - len(common_benchmarks) > 0: 
                        warning("Lost " + str(len(comp_benchmarks) - len(common_benchmarks)) +  "/" + str(len(comp_benchmarks)) + " benchmarks, due to inconsistentcy.",year,track,logic)
                    benchmarks = set(db.get_benchmarks()).intersection(set(reverter(b) for b in common_benchmarks))
                    if len(common_benchmarks) - len(benchmarks) > 0: 
                        warning("Lost " + str(len(common_benchmarks) - len(benchmarks)) +  "/" + str(len(common_benchmarks)) + " benchmarks. They are missing from MachSMT's database.",year,track,logic)
    ...
    def mk_plot(self,plot_data,title,loc):
        ...
        for solver in plot_data:
            plt.plot(sorted(plot_data[solver]), label=solver,marker=next(marker),color=next(colors))
        plt.legend()
    ...
    def mk_loss_file(self,plot_data,loc):
        pass
