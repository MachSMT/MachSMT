    ...
    def eval(self):
        ...
        for solver1 in sorted(db.get_solvers()):
            for solver2 in sorted(db.get_solvers()):
                ...
                N+=1
        bar = Bar('Building Pairwise Logic Model', max=N)
        for logic in db.get_logics():
            for solver1 in sorted(db.get_solvers(logic=logic)):
                for solver2 in sorted(db.get_solvers(logic=logic)):
                    X,Y = [], []
                    if solver1 >= solver2: continue
                    bench1, bench2 = set(db.get_benchmarks(solver=solver1,logic=logic)), set(db.get_benchmarks(solver=solver2,logic=logic))
